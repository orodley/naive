...
void write_elf_file(FILE *output_file, AsmModule *asm_module, bool do_link)
{
	...
	header.object_file_type = do_link ? ET_EXEC : ET_REL;
	...
	if (do_link) {
		...
		// We write out the following section headers (in this order):
		//   * NULL header - required by spec, no corresponding section
		//   * .shstrtab
		//   * .text
		//   * .strtab
		//   * .symtab
		//
		// The actual sections are in the same order.
		//
		// @TODO: Add .note.GNU-STACK section to prevent executable stack.
		u32 num_sections = 5;
